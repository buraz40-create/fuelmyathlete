...
export function RecipeCard({
  ...
  return (
    <Link
      ...
      // A row on a phone and a card from md up. As a full card each recipe filled the whole
      // screen, so browsing 64 of them meant 64 screens of scrolling. As a row, four fit at
      // once, which is the difference between browsing and paging.
      className="group flex h-full flex-row overflow-hidden rounded-2xl border border-border bg-surface shadow-sm transition md:flex-col md:rounded-3xl md:hover:-translate-y-1 md:hover:shadow-md"
    >
      {recipe.imageUrl ? (
      ...
      <div className="flex min-w-0 flex-1 flex-col p-3 md:p-5">
        ...
        <h3 className="mt-1 line-clamp-2 text-sm font-semibold leading-snug text-ink md:mt-1.5 md:line-clamp-none md:text-lg">
          {recipe.name}
        </h3>
        {meal && (
        ...
        {recipe.whenToEat && (
          <p className="mt-2 hidden text-xs leading-snug text-muted-foreground line-clamp-2 md:block">
            {recipe.whenToEat}
          </p>
        )}
        <dl className="mt-auto flex items-center gap-3 pt-2 text-[11px] text-muted-foreground md:pt-3">
          ...
          <div className="flex items-center gap-1">
            ...
            <span className="md:hidden">{recipe.servings}</span>
